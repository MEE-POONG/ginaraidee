...
export default function Listbistro({ data, listStore }) {
  ...
  return (
    <div>
      ...
      {menuByStoreList?.map(({ _id, name, price }) => (
        <div key={_id} className="flex items-center justify-center px-3 py-3">
          <div className="max-w-md bg-white rounded-xl overflow-hidden cursor-pointer shadow-lg hover:shadow-2xl hover:scale-105 transform transition-all duration-500 w-full h-48">
