...
export default function Addstoredrink() {
  ...
  const { namestoredrink, detail } = formStoredrink;
  ...
  const deleteStoredrinkById = async (id) => {
    try {
      await Swal.fire({
        icon: 'info',
        ...
        if (e.isConfirmed) {
          await axios.delete('/api/storedrink/' + id)
          await Swal.fire({
            icon: 'success',
  ...
  return (
    <div>
        ...
        <div className="px-4 flex-auto">
          ...
          <form className="w-full max-w-lg" onSubmit={handelSubmit}>
            ...
            <div className="flex space-x-2 p-1 bg-yellow-100 rounded-md mt-2">
              <textarea
                onChange={(e) =>
                  setFormStoredrink({ ...formStoredrink, detail: e.target.value })
                }
                value={detail}
                type="text"
      ...
      <Storedrinklist data={storedrinkList} deleteStoredrinkById={deleteStoredrinkById}/>
    </div>
  )
}
